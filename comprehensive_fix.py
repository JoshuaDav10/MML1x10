#!/usr/bin/env python3
"""
Comprehensive fix for YAML address ordering issues
"""
import re
import sys
...
def main():
    ...
    for i in range(subsegments_start, subsegments_end):
        ...
        address, full_line = parse_function_entry(line)
        if address is not None:
            if function_index < len(function_entries):
                new_lines[i] = f'      {function_entries[function_index][1]}\n'
                function_index += 1
    ...
    for address, full_line in function_entries:
