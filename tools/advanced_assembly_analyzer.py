#!/usr/bin/env python3
"""
Advanced Assembly Analyzer - Complete Program Structure Analysis
This tool properly analyzes the entire 22MB assembly export from Ghidra to extract:
...
class AdvancedAssemblyAnalyzer:
    ...
    def analyze_complete_assembly(self):
        ...
        print("🔍 ADVANCED ASSEMBLY ANALYSIS STARTING")
        ...
        try:
            with open(self.assembly_file_path, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    # Extract line address if present
                    self.extract_line_address(line, line_count)
    ...
    def extract_line_address(self, line, line_num):
        """Extract the address from the beginning of assembly lines"""
        # Look for patterns like: "8001XXXX 10 00 42 ae     sw         v0,0x10(s2)"
        match = re.match(r'^([0-9a-fA-F]{8})\s+', line)
        if match:
            addr = int(match.group(1), 16)
            self.line_addresses[line_num] = addr
    ...
    def is_function_start(self, line):
        ...
        # LAB_8001XXXX
        # glabel FUN_8001XXXX
        return re.search(r'(?:FUN_|func_|LAB_|glabel\s+)([0-9a-fA-F]{8})', line)
    ...
    def extract_function_start(self, line):
        ...
        match = re.search(r'(?:FUN_|func_|LAB_|glabel\s+)([0-9a-fA-F]{8})', line)
        if match:
            addr = match.group(1)
    ...
    def extract_function_end(self, line):
        ...
        if self.current_function:
            ...
            addr_match = re.match(r'^([0-9a-fA-F]{8})\s+', line)
            if addr_match:
...
def main():
    ...
    print("🚀 ADVANCED ASSEMBLY ANALYZER STARTING")
    print("=" * 50)
    
    analyzer = AdvancedAssemblyAnalyzer(assembly_file)
