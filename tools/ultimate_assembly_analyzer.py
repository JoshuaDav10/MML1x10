#!/usr/bin/env python3
"""
Ultimate Assembly Analyzer - Complete Program Structure Analysis
This tool properly analyzes the Ghidra assembly export format to extract:
- All function boundaries (start/end addresses) with proper parsing
...
class UltimateAssemblyAnalyzer:
    ...
    def analyze_complete_assembly(self):
        ...
        print("🔍 ULTIMATE ASSEMBLY ANALYSIS STARTING")
        ...
        print("📋 PASS 1: Collecting function boundaries...")
        self.collect_function_boundaries()
        
        # Second pass: match starts with ends
        print("📋 PASS 2: Matching function boundaries...")
        self.match_function_boundaries()
        
        # Third pass: extract additional information
        print("📋 PASS 3: Extracting additional information...")
        self.extract_additional_info()
        
        return True
    
    def collect_function_boundaries(self):
        """First pass: collect all function starts and ends"""
        line_count = 0
        ...
        try:
            with open(self.assembly_file_path, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    # Look for function ends (jr ra + nop)
                    if self.is_function_end(line):
        ...
        print(f"✅ First pass complete! Processed {line_count:,} lines")
        print(f"📊 Found {len(self.function_starts)} function starts")
        print(f"📊 Found {len(self.function_ends)} function ends")
    ...
    def is_function_end(self, line):
        ...
        # Look for return instructions: jr ra + nop
    ...
    def extract_function_start(self, line):
        ...
        if match:
            ...
            if func_name_match:
                ...
                self.function_starts.append({
                    'name': func_name,
                    'address': addr_int,
                    'line': line.strip()
                })
                
                # Initialize function in our tracking
    ...
    def extract_function_end(self, line):
        ...
        if addr_match:
            ...
            # Look for the nop instruction on the next line
            # For now, just record this as a potential end
            self.function_ends.append({
                'address': end_addr,
                'line': line.strip()
            })
    
    def match_function_boundaries(self):
        """Second pass: match function starts with ends"""
        print("🔍 Matching function boundaries...")
        
        # Sort both lists by address
        self.function_starts.sort(key=lambda x: x['address'])
        self.function_ends.sort(key=lambda x: x['address'])
        
        # Match each function start with the next function start (or end of file)
        for i, func_start in enumerate(self.function_starts):
            start_addr = func_start['address']
            
            # Find the next function start or use a reasonable end
            if i + 1 < len(self.function_starts):
                end_addr = self.function_starts[i + 1]['address']
            else:
                # Last function - estimate end
                end_addr = start_addr + 0x1000  # Default size
            
            # Update function info
            func_addr = f"{start_addr:08X}"
            if func_addr in self.functions:
                self.functions[func_addr]['end'] = end_addr
                self.functions[func_addr]['size'] = end_addr - start_addr
                
                if self.functions[func_addr]['size'] > 0:
                    print(f"✅ Function {func_start['name']}: 0x{start_addr:08X} -> 0x{end_addr:08X} ({self.functions[func_addr]['size']} bytes)")
    
    def extract_additional_info(self):
        """Third pass: extract jump tables, global vars, etc."""
    ...
    def extract_function_call(self, line):
        ...
        if match:
            ...
            # For now, just collect all function calls
            pass
    ...
    def generate_splat_yaml(self):
        ...
        for addr, func_info in sorted_functions:
            if func_info['end'] and func_info['size'] and func_info['size'] > 0:
    ...
    def generate_analysis_report(self):
        ...
        report.append(f"Valid Functions with Boundaries: {valid_functions}")
...
def main():
    ...
    print("🚀 ULTIMATE ASSEMBLY ANALYZER STARTING")
    print("=" * 50)
    
    analyzer = UltimateAssemblyAnalyzer(assembly_file)
